...
for i in range(ncoils):
    coils_i = coils.copy()
    coils_i.pop(i)
    Jenergy.append(CoilEnergy(coils[i], coils_i, regularization_rect(0.05,0.05)))

# Total objective function 
JF = Jf \
    + LENGTH_WEIGHT * sum(Jls) \
    + WEIGHT_LINK * QuadraticPenalty(linkNum)
if quad_pen:
    JF += ENERGY_WEIGHT * sum(QuadraticPenalty(Jenergy[i]) for i in range(ncoils))
else: 
    JF +=  ENERGY_WEIGHT * sum(Jenergy) 
if FORCE_WEIGHT.__float__() != 0:
    Jforce_list = [ForceOpt(coils[i], coils[:i]+coils[i+1:], regularization_rect(0.1,0.1))
               for i in range(0, len(coils))]
    JF +=   FORCE_WEIGHT  * sum(QuadraticPenalty(J) for J in Jforce_list)
...
def fun(dofs):
    ...
    #force_string = ", ".join(f"{J.J():1e}" for J in Jforce_list)
    ...
    #outstr += f", Fmax = sum([{force_string}])={sum(J.J() for J in Jforce_list):1e}"
...
curves_to_vtk(curves, OUT_DIR + f"curves_opt_short_indiv_E9")
pointData = {"B_N": np.sum(bs.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)[:, :, None]}
...
if init_guess:
    ...
    curves_to_vtk(curves, OUT_DIR + f"curves_opt_long_indiv")
    pointData = {"B_N": np.sum(bs.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)[:, :, None]}
...
if compute_forces:
    ...
    for i in range(ncoils):
        ...
        f_int_max[1,i] = jsp.integrate.trapezoid(np.linalg.norm(force_pure,ord=2, axis=1), dx=dphi)
    ...
    pos = np.concatenate([coils[k].curve.gamma()[1:-2] for k in range(ncoils)], axis=0)
    vect = np.concatenate(array_forces, axis=0)

    polydat = pv.PolyData(pos)
    polydat['forces'] = vect

    polydat.save(OUT_DIR+ 'forces_opt_indiv_E9.vtk')
