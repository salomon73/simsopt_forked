...
energy_weight_range = np.logspace(-13, -9, 20)
flux_weight_range = [1] 
length_weight_range = [0] # np.append(0,np.logspace(-8,-5,4)) 
        ...
        for lw in length_weight_range:
            for fw in flux_weight_range:
                ...
                result = {
                    ...
                    'max_force': np.max([float(np.max(np.linalg.norm(coil_force(c, coils, regularization_rect(reg, reg)), axis=1))) for c in coils[0:ncoils]]),
                    'int_force': np.sum([float(np.mean(c.curve.incremental_arclength()) \
...
res_file = OUT_DIR + "results_correlation_study.csv"

with open(res_file, 'w', newline='') as csvfile:
    fieldnames = ['energy_weight', 'length_weight','flux_weight', 'reg', 'total_energy', 'total_length', 'total_flux', 'BdotN', 'avg_B', 'rel_B', 'max_force', 'int_force', 'arclength']
