...
MAXITER = 50 if in_github_actions else 2500
SAVE_EVERY = 1
...
print("Filamentary coils saved to " +  OUT_DIR + "curves_init.vtu" +
       " and rectangular coils saved to "+  OUT_DIR + "curves_init_rect.vtp" )

if animate: 
    # Initial coil set for animation
    fname_vid = OUT_DIR + f"coils_iter_00000.vtp"

    coils_to_rectangular_vtk(coils, fname_vid, w=0.4, h=0.4, 
                                planar = True, extra_data=extra_data, useRMF=False)
    print("Saved initial coils set for animation under " + fname_vid)
...
E_total = sum(J.J() for J in Jenergy)  
