...
np.random.seed(1)
eps_vec = np.flip(np.logspace(-12,-3,n_tests))
err_vec = np.zeros((n_tests, n_tests))
...
ax.set_ylabel(r'$\frac{|E(\mathbf{x}+\epsilon\mathbf{h})-E(\mathbf{x}-\epsilon\mathbf{h})|}{|2\epsilon dE|}$', fontsize=30)
